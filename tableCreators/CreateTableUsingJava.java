...
public class CreateTableUsingJava {
	...
	public static void main(String[] args) {
		...
		try {
			...
			db1.execute();
			PreparedStatement createHod = conn.prepareStatement("create table HOD(HodId int primary key auto_increment,HodFirstName varchar(15),HodLastName varchar(15),HodCity varchar(12),HodEmail varchar(20) unique,HodPassword varchar(15))");
			createHod.execute();
			createHod = conn.prepareStatement("alter table HOD auto_increment=5000");
			createHod.execute();
			PreparedStatement createEmp = conn.prepareStatement("create table Employee(employeeId int primary key auto_increment,empFirstName varchar(15),empLastName varchar(15),empCity varchar(12),empSalary int,empEmail varchar(20) unique,empPassword varchar(15))");
			createEmp.execute();
			createEmp = conn.prepareStatement("alter table Employee auto_increment=1000");
			createEmp.execute();
			PreparedStatement createEng = conn.prepareStatement("create table Engineer(EngineerId int primary key auto_increment,engFirstName varchar(15),engLastName varchar(15),engCity varchar(12),engSalary int,engEmail varchar(20) unique,engPassword varchar(15),engType varchar(12),hodId int)");
			createEng.execute();
